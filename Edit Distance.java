 ...
 * Function:
 *  因为 word1进行一个insert操作 与 word2进行一次remove操作 其实是相同的，故这边仅仅针对A的操作进行分析
 *  当 word1[i] == word2[j] 时：
 *      dp[i][j] = dp[i-1][j-1]
 *  当 word1[i] != word2[j] 时：
 *      进行 replace 操作时：dp[i][j] = dp[i-1][j-1] + 1
 *      进行 insert 操作时：dp[i][j] = dp[i-1][j] + 1
 *      进行 remove 操作时：dp[i][j] = dp[i][j-1] + 1
